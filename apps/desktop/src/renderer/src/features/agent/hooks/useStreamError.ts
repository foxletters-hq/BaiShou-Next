...
export function useStreamError(error: string | null, isStreaming: boolean): void {
  ...
  const getLocalizedError = (rawErr: string): string => {
    ...
    if (
      ...
      ((lower.includes('not support') || lower.includes('does not support')) && (lower.includes('capability') || lower.includes('part') || lower.includes('content')))
    ) {
      return t('agent.error.vision_not_supported', '当前选中的模型不支持图片识别/视觉多模态能力，请更换模型后再试。');
    }
    ...
    if (lower.includes('terminated') || lower.includes('econnreset') || lower.includes('socket hung up')) {
      return t('agent.error.terminated', '网络连接意外断开 (ECONNRESET)，请检查您的代理稳定性，系统已为您保存当前已生成的部分内容，您可以尝试重新生成。');
    }
