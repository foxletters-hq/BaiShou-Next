...
function node(partial: Partial<GraphPageNode> & Pick<GraphPageNode, 'id'>): GraphPageNode {
  return { nodeType: 'person', reviewStatus: 'approved', ...partial }
}

function edge(
  partial: Partial<GraphPageEdge> & Pick<GraphPageEdge, 'id' | 'fromId' | 'toId'>
): GraphPageEdge {
  return { reviewStatus: 'approved', ...partial }
}

describe('GRAPH_FILTER_NODE_TYPES', () => {
  it('should exclude the structural diary entry type', () => {
...
describe('filterGraphPageDisplayNode', () => {
  it('should drop rejected nodes and hidden diary anchors', () => {
    expect(
      filterGraphPageDisplayNode(node({ id: 'r', reviewStatus: 'rejected' }), emptyHighlight)
    ).toBe(false)
    expect(filterGraphPageDisplayNode(node({ id: 'e', nodeType: 'entry' }), emptyHighlight)).toBe(
      false
    )
    expect(
      filterGraphPageDisplayNode(node({ id: 'e', nodeType: 'entry' }), {
        ...emptyHighlight,
        hideEntry: false
      })
    ).toBe(true)
  ...
  it('should drop pending nodes in approved-only mode unless selected or located', () => {
    ...
    expect(filterGraphPageDisplayNode(pending, { ...emptyHighlight, approvedOnly: true })).toBe(
      false
    )
    expect(
      filterGraphPageDisplayNode(pending, {
        ...emptyHighlight,
        approvedOnly: true,
        selectedId: 'p',
  ...
  it('should hide disabled entity types except when the node is located', () => {
    ...
    expect(
      filterGraphPageDisplayNode(place, {
        ...emptyHighlight,
...
describe('buildGraphPageDisplayNodes', () => {
  it('should use the neighborhood subgraph when it is pinned', () => {
    const result = buildGraphPageDisplayNodes({
...
describe('buildGraphPageDisplayEdges', () => {
...
describe('buildGraphPageDetailEdges', () => {
  it('should list incident edges and skip rejected ones', () => {
    ...
    const list = buildGraphPageDetailEdges({
      selectedId: 'n1',
      localView: null,
      edges,
...
describe('countGraphCanvasNodes', () => {
  it('should ignore rejected nodes', () => {
    expect(
      countGraphCanvasNodes([
