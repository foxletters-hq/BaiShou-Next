...
describe('empty / phase flags', () => {
  ...
  it('should pick boot / awaken / main from the gate flags', () => {
    expect(graphScreenPhaseKey({ awakenPending: true, showAwakenGate: false })).toBe('boot')
    expect(graphScreenPhaseKey({ awakenPending: false, showAwakenGate: true })).toBe('awaken')
    expect(graphScreenPhaseKey({ awakenPending: false, showAwakenGate: false })).toBe('main')
  })
})
...
describe('findGraphScreenNode', () => {
  it('should prefer the global list and fall back to pending nodes', () => {
    expect(findGraphScreenNode('p', [{ id: 'a' }], [{ id: 'p', name: '待审' }])).toEqual({
      id: 'p',
      name: '待审'
    })
    expect(findGraphScreenNode('missing', [{ id: 'a' }], [])).toBeNull()
  })
})
