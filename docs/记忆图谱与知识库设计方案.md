...
> **「文件禁止向量」已复核并维持（2026-08-03）**
>
> 设计知识库时曾评估过把向量写进同步文件，结论是**否决**，此铁律对所有原始数据源一律适用，知识库同样遵守。
>
> 否决要点：重新嵌入的成本远低于直觉（一份 100 页文档不到 1 分钱、约 7 秒），而存向量会带来换模型触发全量重传、vault 数据绑定模型版本、索引与正文可能不一致等代价。真正昂贵、值得固化同步的是 **OCR 提取结果**，不是向量。完整论证见 [知识库K1-向量笔记本方案.md](./知识库K1-向量笔记本方案.md) §D9。
>
> 对 `memory` / `graph` 还有一层更强的理由：它们的内容**可变**（记忆会被编辑、合并、调重要度），内容一改向量就失效，任何分片设计都救不回来。
...
export class GraphRepository {
  ...
  async searchNodesByName(
    vaultName: string,
    query: string,
    opts?: { nodeTypes?: GraphNodeType[]; limit?: number }
