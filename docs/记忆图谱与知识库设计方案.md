...
旁路：`Memory|Graph/shards.manifest.json`（MD5）；可选 `Graph/nodes.idmap.json`。
...
| P1 | 图 schema + Repo + GraphRawManager + 先文件后灌库 | 能存能查能同步 |
| P2 | LLM + pending-reextract + 按篇替换 + 待确认 | 图有数据 |
| P3 | 图管理页 | 看得见、管得着 |
| P4 | `recall_relations` | 给 AI 用 |
| P5 | 集合类**行级 LWW 合并**（§2.5.3） | 双端同月不互抹 |
| K1 | 知识库（未排期） | 学习场景 |

P0 → P1–P3 → P4；P5 可与图谱并行但依赖 Manager/月分片已落地。
...
- 生产写入走 `HybridSearchEmbeddingStore`；缺 `listSourceIdsByType`。
...
11. ✅ 不对 JSONL 照搬 Remotely Save 的 markdown diff3；整文件也不上 Git。
