/**
 * 仓库稳定 ID 工具（V2.1 / V2.2 共享）。
 *
 * 放在 `@baishou/shared`：`@baishou/database` 回填不能依赖 `@baishou/core`
 *（core → database，避免循环依赖）。
 *
 * 移动端 Hermes 无可靠的 `node:crypto`，因此哈希走纯 JS SHA-256。
...
export function isVaultId(value: unknown): value is string {
  return (
    typeof value === 'string' &&
    value.startsWith(VAULT_ID_PREFIX) &&
    value.length > VAULT_ID_PREFIX.length
  )
}
