...
describe('HotkeySettingsCard', () => {
  it('关闭时仍显示右侧展开按钮', () => {
    const { container } = render(<HotkeySettingsCard config={disabledConfig} onChange={vi.fn()} />)
  ...
  it('开启时仍显示右侧展开按钮', () => {
    const { container } = render(<HotkeySettingsCard config={enabledConfig} onChange={vi.fn()} />)
    expect(container.querySelector('.settings-expansion-toggle')).toBeInTheDocument()
  })
})
